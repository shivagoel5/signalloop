...
// API, so one request shape covers them. Groq is tried first by default and Gemini is the fallback when
// it fails (rate limit, error, timeout); a call can prefer a provider instead, and still falls back to
// the other. Keys come from environment variables and are never logged.
...
export class LLMClient {
  ...
  // Returns the assistant message. Falls through providers in order on any failure. `prefer` names a
  // provider to try first for this call only; it does not change the active provider.
  async chat({ messages, tools, jsonSchema, maxTokens = 1500, prefer }) {
    const failures = [];
    const queue = prefer
      ? [...this.providers.filter((p) => p.name === prefer), ...this.providers.filter((p) => p.name !== prefer)]
      : this.providers.slice(this.active);
    for (const p of queue) {
      try {
        const { message, usage } = await this.#request(p, { messages, tools, jsonSchema, maxTokens });
        if (!prefer) this.active = this.providers.indexOf(p);
