...
// PMM strategy + campaign evidence -> Marketing Agent (recommendation, approved by the marketer) -> Content Agent
// (variant, approved by the marketer) -> HubSpot distribution -> simulated measurement -> deterministic analytics
// -> learning that the next recommendation must build on.

import { CONTACTS, OBJECTIVES, getAudience } from "./company.mjs";
...
export async function runExperiment({ profile, companyKey, session, spec, hubspot, crmState = {}, seed, now = new Date() }) {
  ...
  // How many earlier experiments already sent each audience a given message (messages wear out with repetition).
  const exposures = (audienceId, angle) => session.experiments
    .filter((e) => e.cells.some((c) => c.audienceId === audienceId && c.messagingAngle === angle)).length;

  const rng = makeRng(seed);
  const cells = spec.cells.map((c) => {
    const measured = simulateCell({ companyKey, cell: c, share: c.share, rng, exposures: exposures(c.audienceId, c.messagingAngle) });
    return {
      cellId: c.cellId,
      role: c.role,
...
// Planning step 1: the Marketing Agent recommends what to test next, for the session's current objective. The
// marketer reviews it before any content is created.
export async function recommendStrategy({ profile, session, llm }) {
  const analytics = computeAnalytics({ profile, experiments: session.experiments, objectiveId: session.objectiveId });
  const metric = OBJECTIVES[session.objectiveId].primaryMetric;
  const marketing = await runMarketingAgent({
    llm, profile, analytics, experiments: session.experiments,
    controlFor: (audienceId, channel) => controlFor(profile, session.experiments, audienceId, channel, metric),
  });
...
// A short record of a recommendation, kept when the marketer re-runs it under another objective.
export function planSummary(marketing) {
  const r = marketing.recommendation;
  return {
    objectiveId: marketing.objectiveId,
    objectiveLabel: OBJECTIVES[marketing.objectiveId]?.label ?? marketing.objectiveId,
    priorityAudience: r.priorityAudience,
...
// Planning step 2: the Content Agent plans and writes the variant for the approved recommendation. The next
// experiment (new variant vs the current control) is then ready to run.
export async function createContent({ profile, session, llm }) {
  ...
  const objectiveId = pending.marketing.objectiveId ?? session.objectiveId;
  const analytics = computeAnalytics({ profile, experiments: session.experiments, objectiveId });
  ...
  const control = controlCell(profile, session.experiments, rec, OBJECTIVES[objectiveId].primaryMetric);

  const spec = {
    kind: "test_vs_control",
    cells: [test, control],
    marketingAgentRecommendation: { ...rec, objectiveId, provider: pending.marketing.provider, model: pending.marketing.model },
...
// Control = the best-performing variant so far on the objective's primary signal for the same audience and channel,
// re-run on the other half of the audience. Falls back to the audience's baseline variant.
function controlCell(profile, experiments, rec, metric) {
  const audience = getAudience(profile, rec.priorityAudience);
  const best = bestVariant(experiments, rec.priorityAudience, rec.recommendedChannel, metric);
...
// The best-performing variant so far (pooled rate on one signal) for an audience on a channel, if any.
function bestVariant(experiments, audienceId, channel, metric) {
  const byVariant = new Map();
  for (const e of experiments) {
    for (const c of e.cells) {
      if (c.audienceId !== audienceId || c.channel !== channel) continue;
      const agg = byVariant.get(c.contentVariantId) ?? { cell: c, variant: e.variants?.[c.contentVariantId], reach: 0, events: 0 };
      agg.reach += c.reach;
      agg.events += c[metric] ?? 0;
      byVariant.set(c.contentVariantId, agg);
    }
  }
  return [...byVariant.values()].sort((a, b) => b.events / (b.reach || 1) - a.events / (a.reach || 1))[0];
}

// The messaging angle and content type the control would use for an audience on a channel.
export function controlFor(profile, experiments, audienceId, channel, metric = "clicks") {
  const audience = getAudience(profile, audienceId);
  if (!audience) return null;
  const best = bestVariant(experiments, audienceId, channel, metric);
  return {
    messagingAngle: best?.cell.messagingAngle ?? audience.default_angle,
    contentType: best?.cell.contentType ?? audience.default_content_type,
  };
...
// Write the audiences in this experiment to HubSpot (live or mock): persona property, last newsletter/campaign
// properties, one static list per audience.
