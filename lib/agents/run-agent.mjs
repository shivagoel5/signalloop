...
export async function runAgent({ llm, system, task, toolbox, finalInstruction, jsonSchema, validate, requiredTools = [], decisionMaxTokens = 2000 }) {
  const messages = [
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const { message, provider, model } = await llm.chat({ messages: decisionMessages, jsonSchema, maxTokens: decisionMaxTokens });
    let output;
    let errors;
    try {
