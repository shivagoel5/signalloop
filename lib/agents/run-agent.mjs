...
//            attempt, then one attempt on the other provider, then fails loudly. An agent can
//            send this phase to a preferred provider first (decisionProvider).
...
export async function runAgent({ llm, system, task, toolbox, finalInstruction, jsonSchema, validate, requiredTools = [], decisionMaxTokens = 2000, decisionProvider }) {
  const messages = [
  ...
  const attempts = [];
  let prefer = decisionProvider;
  for (let attempt = 0; attempt < 3; attempt++) {
    const { message, provider, model } = await llm.chat({ messages: decisionMessages, jsonSchema, maxTokens: decisionMaxTokens, prefer });
    let output;
    let errors;
    try {
    ...
    // Second failure: give the other provider one try.
    if (attempt === 1) {
      const other = llm.providers.find((p) => p.name !== provider);
      if (!other) break;
      prefer = other.name;
    }
  }
