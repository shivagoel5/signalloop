...
**Product marketers can produce a campaign faster than ever, and that is exactly the problem.** When another email or landing page costs almost nothing to make, the bottleneck moves upstream: deciding *which audience, channel and message deserve the next test, and why.* Campaign dashboards report what happened; they rarely say what was learned, or which assumption changed. SignalLoop is a working prototype of that decision, built for product marketers at B2B SaaS companies who own positioning and messaging and have to justify the next experiment with evidence.

I designed the PMM workflow and experiment framework, then built it: deterministic code measures each objective on the signal that objective actually needs, a **Marketing Agent** reads those results alongside the strategy — ICP, buying roles, positioning, message hypotheses — to recommend the next test and name the knowledge gap it closes, a marketer approves, and a **Content Agent** writes the approved asset. Every recommendation has to cite a strategy input, a metric and the last experiment's result, or the code rejects it. Building it taught me that the hard constraint is not model quality but where you put the human: the moment AI was allowed to pick the audience or declare a winner, the output got confident and unfalsifiable — so the strongest design decision in the project is what the agents are *forbidden* to do.
