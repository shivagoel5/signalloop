...
(function(){
  ...
  var TIPS={setup:'Start here. Run a baseline to see how each audience responds.',results:'Next, ask the Marketing Agent what to test.',strategy:'Happy with the plan? Have the Content Agent write it.',content:'Run the new version against the current control.'};
  var STEP_HINTS={setup:'Choose a company and campaign objective, then run a baseline',results:'What the simulated campaign showed, calculated by code',strategy:'The Marketing Agent\'s recommendation for the next test',content:'The Content Agent\'s variant, previewed on its channel'};
  ...
  var TOUR=[
    ...
    ['demo-f-objective','Campaign objective','What the campaign should achieve. Awareness and traffic favor click volume; engagement, consideration and conversion favor response rate.'],
  ...
  function action(label,route,hint,blocked){
  ...
  async function act(route){
    ...
    var res=await api('POST',route,{objective:objSel.value||undefined});
    busy=false;
  ...
  objSel.addEventListener('change',function(){
    ...
    renderAll();
    if(state&&state.pendingPlan)note('The new objective applies to the next recommendation.');
  });
