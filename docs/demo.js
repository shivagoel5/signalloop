...
// Results. Nothing is chosen when the page opens: the visitor picks a company and a campaign objective first, and
// every row of the frame says what it shows. Opening the page or choosing a company makes no request. Only Run,
...
(function(){
  ...
  // Visible one-line descriptions: what each step shows, what each objective favors (see OBJECTIVES in
  // lib/company.mjs) and what each source label means.
  var STEP_SUBS={setup:'Choose, then run a baseline',results:'Performance readout',strategy:'Next-best test',content:'Creative execution'};
  var OBJ_HELP={awareness:'Favors reach and total clicks over response rate',traffic:'Favors click volume over response rate',engagement:'Favors the strongest response rate (CTR)',product_consideration:'Favors strong response from high-intent audiences; volume is secondary',conversion:'Favors the highest proven CTR and avoids unproven bets'};
  ...
  function renderAll(){
    ...
    renderFields();renderLabels();renderStepper();renderPanel();renderHistory();syncControls();
  }
  ...
  // Marks which of the two choices is made, points at the one still missing, and describes the chosen objective.
  function renderFields(){
    var hasObj=!!objSel.value,fc=$('demo-f-company'),fo=$('demo-f-objective');
    fc.classList.toggle('done',!!company);fc.classList.toggle('need',!company);
    fo.classList.toggle('done',hasObj);fo.classList.toggle('need',!!company&&!hasObj);
    objSel.classList.toggle('empty',!hasObj);
    $('demo-objective-d').textContent=hasObj?OBJ_HELP[objSel.value]:'The goal the Marketing Agent weighs results against';
  }
  function renderLabels(){
    var labels=(state&&state.labels)||baseLabels(),used={},legend=$('demo-legend');
    labelsEl.textContent='';legend.textContent='';
    LABELS.forEach(function(l){
      var v=labels[l[0]];if(!v)return;used[v]=true;
    ...
    VALUE_HELP.filter(function(h){return used[h[0]]}).forEach(function(h,i){
      if(i)legend.appendChild(document.createTextNode(' · '));
      legend.appendChild(el('b',null,h[0]));legend.appendChild(document.createTextNode(' = '+h[1]));
    });
  }
  function renderStepper(){
    ...
    STAGES.forEach(function(s,i){
      ...
      var tx=el('span','step-tx');tx.appendChild(el('span','step-t',s[1]));tx.appendChild(el('span','step-s',STEP_SUBS[s[0]]));
      b.appendChild(el('span','step-n',i<cur?'✓':String(i+1)));b.appendChild(tx);
  ...
  // A blocked action stays disabled, its hint says what is missing, and the fields at the top point at it.
  function action(label,route,hint,blocked){
    ...
    if(!blocked&&TIPS[stage]&&!tipsSeen()[stage]){
  ...
  function renderSetup(live){
    if(!company||!objSel.value){
      var info=company&&STATIC.companies[company],obj=objSel.value&&find(STATIC.objectives,objSel.value),list=el('ul','choose');
      head('setup','Choose a company and a campaign objective','Use the two controls at the top of this frame. Nothing is sent to HubSpot or the AI agents until you run the baseline.');
      var c=el('li',company?'ok':null);
      c.appendChild(el('b',null,company?'1 · '+info.name+' ✓':'1 · Company scenario'));
      c.appendChild(el('span',null,company?'Audiences '+SEGMENTS[company]+': '+info.audiences.map(function(a){return a.name}).join(', ')+'.':'Ramp models audiences by role; Square by business stage. Both are demonstration assumptions.'));
      var o=el('li',obj?'ok':null);
      o.appendChild(el('b',null,obj?'2 · '+obj.label+' ✓':'2 · Campaign objective'));
      o.appendChild(el('span',null,obj?OBJ_HELP[obj.id]+'.':'Awareness, traffic, engagement, product consideration or conversion. It sets whether the Marketing Agent favors click volume or response rate.'));
      list.appendChild(c);list.appendChild(o);panel.appendChild(list);
